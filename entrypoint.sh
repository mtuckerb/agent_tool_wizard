...
# Environment setup - FIXED LINE 6
...
if python -c "
try:
    from pipelines.router.pipeline_wrapper import app, AGENT_PORT
    print(f'✅ Agent app imports successfully on port {AGENT_PORT}')
...
# Start the app
exec uvicorn pipelines.router.pipeline_wrapper:app \
    --host 0.0.0.0 \
    --port $AGENT_PORT \
    --log-level info
