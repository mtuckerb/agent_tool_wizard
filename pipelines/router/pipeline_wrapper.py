#!/usr/bin/env python3
"""
agent_multitool - LLM-powered MCP Router with Full MCP Integration
Enhanced for multi-provider proxy architecture
"""
...
class LLMConfig:
    ...
    # Proxy configuration for future multi-provider support
    proxy_url: Optional[str] = None
    fallback_providers: Optional[List[str]] = None

class StreamingLLMClient:
    """Streaming LLM client with multi-provider proxy architecture"""
    ...
    def __init__(self, config: LLMConfig):
        ...
        self.fallback_providers = config.fallback_providers or []
    ...
    async def chat_completion_stream(self, messages: List[Dict], temperature: float = 0.1) -> AsyncGenerator[str, None]:
        """Stream chat completion with automatic fallback support"""
    ...
    async def _openai_stream(self, messages: List[Dict], temperature: float):
        ...
        if self.proxy_url:
            endpoint_url = f"{self.proxy_url}/chat/completions"
    ...
    async def _ollama_stream(self, messages: List[Dict], temperature: float):
        ...
        if self.proxy_url:
            endpoint_url = f"{self.proxy_url}/api/chat"
...
class MCPClient:
    ...
    async def _start_server(self, server_name: str, server_config: Dict[str, Any]):
        ...
        # Build the command and arguments list
        if isinstance(command, str):
            cmd_list = [command] + args
        else:
            cmd_list = list(command) + args if command else args
        
        # Merge with current environment, ensuring HOME is set correctly
        server_env = os.environ.copy()
        server_env.update(env)
        # CRITICAL: Ensure HOME is set to agent's home directory
        server_env["HOME"] = "/home/agent"
        
        logger.info(f"Starting MCP server {server_name}: {' '.join(cmd_list)}")
    ...
    async def _initialize_server_connection(self, server_name: str, process):
        ...
        # Read response with proper timeout and line handling
        try:
            ...
            if response_line:
                ...
                logger.error(f"No response from {server_name} during initialization")
        except asyncio.TimeoutError:
            logger.error(f"Timeout waiting for initialization response from {server_name}")
        except Exception as e:
            ...
            try:
                ...
                if response_line:
                    ...
                    if "result" in response:
                        ...
                        logger.info(f"Found {len(tools[name])} tools from {name}")
                    else:
                        logger.error(f"Invalid response from {name}: {response}")
                        tools[name] = []
                else:
                    logger.error(f"No response from {name} during tools/list")
                    tools[name] = []
                        
            except asyncio.TimeoutError:
                logger.error(f"Timeout waiting for tools from {name}")
                tools[name] = []
            except json.JSONDecodeError as e:
                logger.error(f"Failed to decode tools response from {name}: {e}")
                tools[name] = []
            except Exception as e:
...
# ==================== ENHANCED ROUTER WITH MCP INTEGRATION ====================

class EnhancedHaystackRouter:
    """Enhanced router with LLM streaming and full MCP integration"""
    
    def __init__(self):
        # Load configuration
        self.config = self._load_config()
        
        # Initialize LLM client with proxy-ready configuration
        llm_config = LLMConfig(
            ...
            fallback_providers=os.getenv("LLM_FALLBACK_PROVIDERS", "").split(",") if os.getenv("LLM_FALLBACK_PROVIDERS") else []
        )
    ...
    async def initialize(self):
        ...
        logger.info(f"Initialized {len(tools)} MCP servers with {tool_count} tools")
    ...
    async def route_request_stream(self, user_input: str, use_tools: bool = False) -> AsyncGenerator[str, None]:
        """Main streaming routing with full MCP integration"""
        ...
        try:
            if use_tools:
                ...
                # Simple LLM routing (original behavior)
                messages = [
    ...
    async def _route_with_tools(self, user_input: str) -> AsyncGenerator[str, None]:
        """Route request with full MCP tool integration"""
        
        yield json.dumps({
            "stage": "tool_discovery",
            "message": "Discovering and connecting to available tools"
        })
        ...
        # Simple tool selection for now - in future, use LLM to select tools
        # For demonstration, we'll try to detect time-related queries
        if any(keyword in user_input.lower() for keyword in ["time", "date", "current"]):
            yield json.dumps({
                "stage": "tool_selection",
                "message": "Detected time-related query, attempting to use time tools"
            })
            
            # Try to find and use time tools
            time_tools_found = False
            for server_name, server_tools in tools.items():
                for tool in server_tools:
                    if "time" in tool.get("name", "").lower() or "current" in tool.get("name", "").lower():
                        try:
                            yield json.dumps({
                            ...
                            result = await self.mcp_client.call_tool(server_name, tool["name"], {})
            ...
            if not time_tools_found:
                yield json.dumps({
                    "stage": "tool_not_found",
                    "message": "No time tools available, falling back to LLM"
                })
...
For now, just provide a helpful response about what's possible."""
        ...
        async for chunk_str in self.llm_client.chat_completion_stream(messages):
            ...
            yield json.dumps({
                "stage": "llm_with_tools_streaming",
                "content": content,
                "provider": chunk.get("provider")
            })
...
async def root():
    ...
    return {
        ...
        "version": AGENT_VERSION,
        "port": AGENT_PORT,
        "description": "Enhanced LLM-powered MCP Tool Router with Full Integration",
        "endpoints": {
            "health": f"/api/health",
            "router": f"/api/router",
            "tools": "/api/tools",
    ...
    try:
        ...
        return {
            ...
            "llm": {
                ...
                "proxy_ready": bool(current_router.llm_client.proxy_url)
            }
        }
    except Exception as e:
        return {
...
async def route_request(request: RouterRequest):
    """Main router endpoint with streaming and tool integration"""
    current_router = await get_router()
...
async def mcp_handler(request: dict):
    """Enhanced MCP JSON-RPC endpoint with full tool integration"""
    try:
        current_router = await get_router()
        ...
        if method == "initialize":
            ...
            all_tools.append({
                ...
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "user_query": {"type": "string", "description": "The user's request or question"},
                        "available_tools": {"type": "array", "items": {"type": "object"}, "description": "List of available tools to consider (auto-populated)"}
                    },
                    "required": ["user_query"]
                }
            })
...
# OpenAI-compatible endpoints (enhanced with proxy support)
@app.post("/v1/chat/completions")
async def openai_compatible_chat(request: dict):
    """OpenAI-compatible chat completions with proxy support"""
    try:
        current_router = await get_router()
